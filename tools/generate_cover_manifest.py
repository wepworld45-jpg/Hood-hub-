...
def update_index_manifest(critical: dict) -> None:
    html = INDEX.read_text(encoding="utf-8")
    payload = json.dumps(critical, ensure_ascii=False, separators=(",", ":"))
    block = "<!-- VIVUHUB_CRITICAL_MANIFEST_START -->\n<script>window.VIVUHUB_COVER_MANIFEST=" + payload + ";</script>\n<!-- VIVUHUB_CRITICAL_MANIFEST_END -->"
    pattern = re.compile(r"<!-- VIVUHUB_CRITICAL_MANIFEST_START -->.*?<!-- VIVUHUB_CRITICAL_MANIFEST_END -->", re.S)
...
def main() -> int:
    ...
    update_index_manifest(critical)
    print(json.dumps({
